...
const ProductCard = ({ product, customerId }) => {
  ...
  useEffect(() => {
    const checkIfFavorite = async () => {
      try {
        const response = await axiosInstance.get(`/data/wishlist/${customerId}/${product_id}`);
  ...
  const handleFavoriteClick = async () => {
    try {
      ...
      if (!favorite) {
        await axiosInstance.post('/data/wishlist', {
        ...
        await axiosInstance.delete(`/data/wishlist/${customerId}/${product_id}`);
        setFavorite(false);
  ...
  const handleAddToCart = async () => {
    ...
    try {
      ...
      await axiosInstance.post('/data/cart', {
        customerId: customerId,
        productId: product_id,
